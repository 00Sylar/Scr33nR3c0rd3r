...
| **Max Player tiles** *(default UI only)* | Caps how many tiles can be open at once in the **▶ Player** tab (1–20, default 9). Every open tile streams live (muted), so this is also a bandwidth/CPU cap — lower it if opening many tiles strains your connection. |
