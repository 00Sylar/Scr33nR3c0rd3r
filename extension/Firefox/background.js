...
let TOKEN = '';
const tokenReady = browser.storage.local.get('apiToken')
...
async function updateGlobalBadge(data) {
  ...
  try {
    await browser.action.setBadgeBackgroundColor({ color: '#d32f2f' });
...
async function updateTabBadge(tabId, url, data) {
  ...
  try {
    if (!m) {
      ...
      await browser.action.setBadgeText({ tabId, text: null });
    ...
    await browser.action.setTitle({ tabId, title: `Scr33nX — ${m.name}: ${state}` });
  } catch {}
}
...
async function refreshAll(force) {
  ...
  let tabs = [];
  try { tabs = await browser.tabs.query({ url: SITE_PATTERNS }); } catch {}
...
browser.tabs.onUpdated.addListener(async (tabId, changeInfo, tab) => {
  ...
  if (!modelFromUrl(tab.url || '')) {
    try { await browser.action.setBadgeText({ tabId, text: null }); } catch {}
    return;
  }
...
browser.alarms.create('scr33nx-poll', { periodInMinutes: 0.5 });
...
function createMenus() {
  // browser.* is promise-based — removeAll takes no callback here.
...
// ── Startup ────────────────────────────────────────────────────────────────────
browser.runtime.onInstalled.addListener(() => { createMenus(); refreshAll(true); });
browser.runtime.onStartup.addListener(() => { createMenus(); refreshAll(true); });
